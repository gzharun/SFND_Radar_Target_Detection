...
td = t - 2*r_t/c;
Rx = cos(2*pi*(fc*td + Slope*td.^2/2));
...
% Dear Reviewer
%noise_level = zeros(1,1); It would be more effective to create a matrix
%for thresholded signal.
...
for i = Gr+Tr+1:(Nr/2-(Gr+Tr+1))
    for j = Gd+Td+1:(Nd-(Gd+Td+1))
        ...
        noise_level(i, j) = pow2db(S) + offset;
